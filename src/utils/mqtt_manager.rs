            ...
            Self::AnnounceAndSubscribe { connection_id } => {
                if connection_id == &current_connection_id && !is_retry {
                    client
                        ...
                        .await?;
                    client.subscribe("my/topic", QualityOfService::Qos1).await?;
                }
            }
...
async fn mqtt_channel_task(
    stack: Stack<'static>,
    uid: &'static str,
    event_sender: Sender<'static, NoopRawMutex, MqttEvent<EmergeMqttEvent>, 32>,
    ...
    mqtt_manager::run::<MqttAction, EmergeMqttEvent, 16, 4096, 32>(
        stack,
        connection_settings,
        settings,
        event_sender,
...
async fn mqtt_task(
    ...
    events_in: Receiver<'static, NoopRawMutex, MqttEvent<EmergeMqttEvent>, 32>,
    events_out: EventPub,
        ...
        match next {
            ...
            Either::Second(event) => match event {
                ...
                MqttEvent::ConnectionStable { .. } => {
                    ...
                    command_sender
                        .send(robot::events::RobotEvents::ConnectedToMqtt(true))
                        .await
                }
                MqttEvent::Disconnected { .. } => {
                    info!("MQTT disconnected");
                    command_sender
                        .send(robot::events::RobotEvents::ConnectedToMqtt(false))
                        .await
                }
                event => {
                    info!("{:?}", event);
...
static ACTION_CHANNEL: StaticCell<Channel<NoopRawMutex, MqttEvent<EmergeMqttEvent>, 32>> = StaticCell::new();
...
pub async fn init(
    ...
    let mqtt_event_channel =
        ACTION_CHANNEL.init(Channel::<NoopRawMutex, MqttEvent<EmergeMqttEvent>, 32>::new());
